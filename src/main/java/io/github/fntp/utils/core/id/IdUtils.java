package io.github.fntp.utils.core.id;

import io.github.fntp.utils.common.enums.FntpCodeEnums;
import io.github.fntp.utils.common.enums.FntpInfoEnums;
import io.github.fntp.utils.common.exception.FntpException;
import io.github.fntp.utils.core.base64.Base64Utils;
...
public class IdUtils {
    ...
    private static byte[] toBytes() {
        ...
        for (int i = 0; i < FntpInfoEnums.ALIYUN_DRIVE_COMMON_NUMBER_8.getEnumsIntegerValue(); i++) {
    ...
    public static String getUuid() {
        return Base64Utils.toBase64String(toBytes());
    }
    ...
    public static String getUuidBits(int bits) {
        ...
        int offsetm;
        int offset;
        for(; bit < FntpInfoEnums.ALIYUN_DRIVE_COMMON_NUMBER_16.getEnumsIntegerValue(); bit += FntpInfoEnums.ALIYUN_DRIVE_COMMON_NUMBER_3.getEnumsIntegerValue(), idx += FntpInfoEnums.ALIYUN_DRIVE_COMMON_NUMBER_4.getEnumsIntegerValue()) {
            offsetm = 64 - (bit + 3) * 8;
            tmp = 0;
            if(bt1 > 3) {
                mask = (1 << 8 * 3) - 1;
    ...
    private static  synchronized long nextId(long datacenterId, long machineId,long lastStamp,long sequence) {
        ...
        if (currStamp < lastStamp) {
            throw new FntpException(FntpCodeEnums.ERROR_PARAM.getCode(), FntpCodeEnums.ERROR_PARAM.getMessage());
        }
        if (currStamp == lastStamp) {
            //相同毫秒内，序列号自增
